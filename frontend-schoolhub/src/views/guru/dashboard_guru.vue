<template>
  <DashboardLayout title="Dashboard Guru" role-label="Guru" :navigation="navigation"
    ><section class="welcome">
      <div>
        <span class="eyebrow-dot dark">Ruang pengajar</span>
        <h2>Selamat datang, {{ user.name || 'Guru' }}.</h2>
        <p>Pantau kelas, tugas, dan perkembangan pembelajaran Anda.</p>
      </div>
    ...
    <div class="insight">
      <article>
        <h2>Analisis kompetensi siswa</h2>
    ...
    <section class="head">
      <h2>Menu pengajaran</h2>
      <p>Akses pekerjaan utama Anda.</p>
    </section>
...
const features = [
  {
    title: 'Data Kelas',
    description: 'Lihat siswa dan informasi kelas.',
    icon: 'fas fa-users',
...
const progress = [
  { label: 'Materi selesai', value: 80 },
  { label: 'Tingkat kelulusan', value: 93 },
  { label: 'Keaktifan siswa', value: 75 },
]
onMounted(() => {
  chart = new Chart(chartCanvas.value, {
    type: 'radar',
    data: {
      labels: ['Logika', 'Kalkulus', 'Geometri', 'Statistika', 'Aljabar'],
      datasets: [
        {
          label: 'Kelas X MIPA 1',
          data: [80, 65, 75, 90, 85],
          backgroundColor: 'rgba(28,156,95,.18)',
          borderColor: '#1c9c5f',
          pointBackgroundColor: '#1c9c5f',
        },
      ],
    },
    options: {
...
.bar i {
  display: block;
  height: 100%;
  background: linear-gradient(90deg, #1c9c5f, #8fd94a);
  border-radius: 99px;
}
.head {
  margin: 32px 0 15px;
